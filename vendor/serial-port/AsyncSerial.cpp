 ...
 * 
 * 
 * Replaced boost::bind with lambdas by Sigurd Saue (Nov 2023)
 */
	...
	asio::serial_port_base::stop_bits opt_stop)
{
	...
	pimpl->io.post([&]() { this->doRead(); });

	thread t([&]() {pimpl->io.run(); });
...
void AsyncSerial::doRead()
{
	pimpl->port.async_read_some(asio::buffer(pimpl->readBuffer, readBufferSize),
		[&](const boost::system::error_code& error,
			size_t bytes_transferred) { this->readEnd(error, bytes_transferred); });
}
...
void AsyncSerial::doWrite()
{
	...
	if (pimpl->writeBuffer == 0)
	{
		...
		async_write(pimpl->port, asio::buffer(pimpl->writeBuffer.get(),
			...
			[&](const boost::system::error_code& error, size_t) { this->writeEnd(error); });
	}
}
